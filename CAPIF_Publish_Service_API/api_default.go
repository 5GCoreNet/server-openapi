/*
 * CAPIF_Publish_Service_API
 *
 * API for publishing service APIs.   © 2022, 3GPP Organizational Partners (ARIB, ATIS, CCSA, ETSI, TSDSI, TTA, TTC).   All rights reserved. 
 *
 * API version: 1.2.1
 * Generated by: OpenAPI Generator (https://openapi-generator.tech)
 */

package openapi_CAPIF_Publish_Service_API

import (
	"net/http"

	"github.com/gin-gonic/gin"
)

// ApfIdServiceApisGet - 
func ApfIdServiceApisGet(c *gin.Context) {
	c.JSON(http.StatusOK, gin.H{})
}

// ApfIdServiceApisPost - 
func ApfIdServiceApisPost(c *gin.Context) {
	c.JSON(http.StatusOK, gin.H{})
}

// ApfIdServiceApisServiceApiIdDelete - 
func ApfIdServiceApisServiceApiIdDelete(c *gin.Context) {
	c.JSON(http.StatusOK, gin.H{})
}

// ApfIdServiceApisServiceApiIdGet - 
func ApfIdServiceApisServiceApiIdGet(c *gin.Context) {
	c.JSON(http.StatusOK, gin.H{})
}

// ApfIdServiceApisServiceApiIdPut - 
func ApfIdServiceApisServiceApiIdPut(c *gin.Context) {
	c.JSON(http.StatusOK, gin.H{})
}
